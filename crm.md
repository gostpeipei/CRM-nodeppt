...
[slide]
1. 线索录入 <i class="fa fa-arrow-down"></i> {:&.rollIn}
2. 创建商机 <i class="fa fa-arrow-down"></i>
...
[slide]
<h2><span class="yellow">2.项目代码结构目录</span></h2>

[slide]
<pre><code class="markdown">/* 项目一级目录 */
  |-- bin/www（脚本入口文件）
  |-- common/（存放了配置路时封装的一些方法）
  ...
  |-- .gitignore (配置不需要上传到代码仓库的文件)
  |-- .jshintrc (js代码规范检测工具配置文件)
  |-- app.js (node express框架的配置文件)
  |-- config.js (项目环境配置文件)
...
[slide]
<h2><span class="yellow">3.项目技术栈</span></h2> 

[slide]
## `前端主要技术栈`

> CRM: 目前项目未前后端分离，前端依赖Node.js环境，后端也由Node.js编写，使用express框架，页面采用服务端渲染。


---
分类| 应用| 备注
:-------|:------:|
模板引擎 | swig | 配合路由，由后端返回对应页面及相关css、js文件
路由 | express.Router() | 后端返回页面html
...
MJJS.ui.iTable('#advTable', {
  ...
  default: (MJJS.common.defaultTable) //定义了接口返回字段的成功状态码,里面数据的读取路径,数据总数.还有传给后台接口的页码,一页数据条数等字段信息
...
2. 建立自己的分支
3. 将自己的代码提交在自己的分支上
4. 之后再merge到Add下中当前的版本分支上
5. 发布测试


[slide]
<h3 style="text-align: left;">开发系统登录地址:</h3>

<h3 style="text-align: left;">生产环境: <span class="yellow" style="font-size: 0.8em;">http:&frasl;&frasl;mj-crm.weimob.com</span></h3>
...
<span style="width:80%; display: inline-block;vertical-align: top;text-align: left;">echart/highchart本质上功能一样,项目中两个都有引入,并且重复引用,lib/和util/custom下都有</span>
</p>
...
<span style="width:80%; display: inline-block;vertical-align: top;text-align: left;">制定严格的规范,给非npm安装的外部依赖固定位置</span>
</p>
...
[slide]
<h3 align="left"><code>问题E</code> ES6没有在项目中很好的使用</h3>
<p align="left">
<span style="width:10%;text-align:left;"><strong>举例: </strong></span>
<span style="width:60%; display: inline-block;vertical-align: top;text-align: left;">只在有些地方见过使用const</span>
<img src="/imgs/issue_e.png" height="250" style="position:absolute;">
</p>
<p align="left">
<span style="width:10%;text-align:left;"><strong>原因: </strong></span>
<span style="width:60%; display: inline-block;vertical-align: top;text-align: left;">考虑兼容性的问题或者没有尝试去使用ES6编写</span>
</p>
<p align="left">
<span style="width:10%;text-align:left;"><strong>解决方案: </strong></span>
<span style="width:55%; display: inline-block;vertical-align: top;text-align: left;">如果是考虑兼容性上的问题,使用gulp + babel可实现ES6代码的编译，如果不需要考虑兼容性问题，可直接书写ES6</span>
</p>
